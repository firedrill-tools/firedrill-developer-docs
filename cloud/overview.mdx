...
| Starting data, scenarios, actors, and drills | Repository-owned | Compiled from an immutable uploaded archive or source draft |
...
<Steps>
  <Step title="Create a project">
    A project contains builds, environments, sessions, runs, source drafts, and
    access grants. Organization membership does not automatically grant access to
    every project.
  </Step>
  <Step title="Produce an immutable build">
    Publish a local build or a reviewed source draft. The build pipeline verifies
    and compiles the selected source.
  </Step>
  <Step title="Point an environment at a build">
    An environment pins future sessions to one immutable build. Promoting a new
    build changes only future sessions.
  </Step>
