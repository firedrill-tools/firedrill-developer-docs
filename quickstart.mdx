...
Using Python? Follow the [local Python quickstart](/sdk/local-python#try-a-drill)
for wheel installation, the same CLI, and pytest integration.
...
<Info>
  The local framework is a release candidate. Install it from the `@firedrill-run` npm scope with the `next` tag.
</Info>
