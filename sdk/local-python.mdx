...
<Note>
  The local Python package is a release candidate. Its PyPI release is pending.
  Use a supplied wheel matching your operating system and architecture; do not
  install the unrelated package named `firedrill`.
</Note>

From the directory containing that single wheel:

<CodeGroup>
```sh macOS / Linux
python -m pip install ./firedrill_run-*.whl pytest
firedrill --help
```

```powershell Windows PowerShell
python -m pip install (Get-ChildItem -File firedrill_run-*.whl).FullName pytest
firedrill --help
```
</CodeGroup>

After the PyPI release is available, the registry installation command will be:
...
Use an initialized project for these commands. The CLI shows the connection
settings and any browser app links. Python uses the same Tool packages as
TypeScript; you do not need separate Python copies. Tool installation uses the
package manager included in the wheel.
...
The `agent` extra provides the Claude Agent SDK companion. Before registry
publication, install it from the directory containing your release wheel:

```sh
python -m pip install --find-links . "firedrill-run[agent]==0.1.0rc1"
